...
urlpatterns = [
    ...
    path("archive/<int:year>/<int:month>/",blog_views.MonthlyArchiveView.as_view(month_format='%m'),name='archive-view'),

    path("post/<int:pk>/", blog_views.SingleBlogView.as_view(), name='single'),
    
    path('post/<int:pk>/comment/', blog_views.PostCommentView.as_view(), name='add-comment'),
]
